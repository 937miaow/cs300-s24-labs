...
inline uintptr_t make_syscall(int syscallno) {
  ...
  1c0004:	b8 01 00 00 00       	mov    $0x1,%eax
    asm volatile ("syscall"
  1c0009:	0f 05                	syscall 
    return x - (x % multiple);
}
template <typename T>
inline constexpr T round_up(T x, unsigned multiple) {
  ...
  1c000b:	b8 0f 20 1c 00       	mov    $0x1c200f,%eax
    pid_t p = sys_getpid();
  ...
  1c0010:	48 25 00 f0 ff ff    	and    $0xfffffffffffff000,%rax
  1c0016:	48 89 05 eb 0f 00 00 	mov    %rax,0xfeb(%rip)        # 1c1008 <heap_top>
    return x;
}
...
__always_inline uintptr_t rdrsp() {
  ...
  1c001d:	48 89 e0             	mov    %rsp,%rax

    // The bottom of the stack is the first address on the current
    // stack page (this process never needs more than one stack page).
    stack_bottom = (uint8_t*) round_down((uintptr_t) rdrsp() - 1, PAGESIZE);
  1c0020:	48 83 e8 01          	sub    $0x1,%rax
  1c0024:	48 25 00 f0 ff ff    	and    $0xfffffffffffff000,%rax
  1c002a:	48 89 05 cf 0f 00 00 	mov    %rax,0xfcf(%rip)        # 1c1000 <stack_bottom>
    register uintptr_t rax asm("rax") = syscallno;
  1c0031:	b8 02 00 00 00       	mov    $0x2,%eax
    asm volatile ("syscall"
  1c0036:	0f 05                	syscall 
            : "+a" (rax)
            : /* all input registers are also output registers */
            : "cc", "memory", "rcx", "rdx", "rsi", "rdi", "r8", "r9",
              "r10", "r11");
    return rax;
  1c0038:	eb f7                	jmp    1c0031 <process_main()+0x31>
